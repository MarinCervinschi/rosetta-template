...
See [How to add a warning callout](/how-to/meta/add-a-warning/) for guidance on when to reach for each variant.
...
See [Mermaid diagrams](/reference/meta/mermaid/) for more diagram types and the fenced code block syntax.

## `<StackIcon />`

Renders a single inline brand icon from [`simple-icons`](https://simpleicons.org/). Designed for stack-style bullet lists on the landing page, but usable anywhere in MDX. Unknown brand names render a small dot placeholder so the bullet still lays out cleanly — no runtime errors.
